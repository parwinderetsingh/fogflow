...
class check_ip_port:
    ...
    def check_fog_end_point(self):
	   ...
	   self.s.connect((self.fog_ip,self.fog_port))
           ...
           print("Connection to %s on port %s failed: %s" % (self.fog_ip, self.fog_port, e))
           return False
    ...
    def check_scarpio_broker_endpoint(self):
        try:           
           ...
           print("Connection to %s on port %s failed: %s" % (self.scarpio_broker_ip, self.scarpio_broker_port, e))
           return False
